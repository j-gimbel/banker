...
class parser {
    ...
    public function addDataFromFiles($files) {
        ...
        to do : add umsatzIds to mandate
    ...
    public function addBuchungsArt ($buchungsArt,$text) {

        $insert = 'INSERT INTO buchungsArten (name, info) VALUES (:name, $text)';
        $stmt = $this->db->prepare($insert);
        $stmt->bindParam(':name', $buchungsArt);
        $stmt->execute(); 
        $buchungsartID = $this->db->lastInsertId();
        $this->lookup['buchungsArten'][$buchungsArt] = array();  
        $this->lookup['buchungsArten'][$buchungsArt]['buchungsartID'] = $buchungsartID;
        $this->lookup['buchungsArten'][$buchungsArt]['umsatzIDs'] = array();
        $this->lookup['buchungsArten'][$buchungsArt]['info'] = $text;
    ...
    private function addMandat($mandatName,$CRED,$MREF) {
        
        #$this->log->debug('addMandat '.$mandatName);
        ...
        $this->lookup['mandate'][$mandatID]['mandatName'] = $mandatName;
        $this->lookup['mandate'][$mandatID]['MREF'] = $MREF;
        $this->lookup['mandate'][$mandatID]['CRED'] = $CRED;
    ...
    public function addBuchung ($buchung) {
      ...
      $stmt->bindParam(':buchungsartID', $buchung["buchungsartID"] );
      $stmt->bindParam(':buchungsGruppenIDs', $buchung["buchungsgruppenID"]);
      ...
      $stmt->bindParam(':mandatID', $buchung["mandatID"]);
      $stmt->bindParam(':kontoauszugdateiID', $buchung["kontoauszugdateiID"]);
      $stmt->execute(); 
        $this->delta += $buchung["changeValue"];
    ...
    public function analyzeKontoAuszugFile($origfile) {
        ...
        $stmt->execute(); 
        $this->kontoauszugdateiID = $this->db->lastInsertId();
        ...
        foreach ($this->currentFile["pages"] as $pageIndex => $dummy) {
            ...
            foreach ($this->currentFile["pages"][$pageIndex]["buchungen"] as $buchungsIndex => $buchung) {
                ...
                foreach ($this->lookup['mandate'] as $possibleMandatID => $dummy)
                {
                  ...
                  $MREF = $this->lookup['mandate'][$possibleMandatID]['MREF'];
                    if ($mandatName == $buchung["mandatName"]) {
                      ...
                      #$this->log->debug('match Name Mandat '.$mandatName);
                      
                      if (($CRED == $buchung["sepa"]["CRED"]) and ($MREF == $buchung["sepa"]["MREF"])) {
                            $buchung["mandatID"] = $possibleMandatID;
                            #print("found ".$mandatName);
                            #exit;
                            #$this->log->debug('match sepa '.$mandatName);
                          break;
                      } 
                  }
                }
                ...
                if ($buchung["mandatID"] == 0) {
                    ...
                    $buchung["mandatID"] = $this->addMandat($buchung["mandatName"],$buchung["sepa"]["CRED"],$buchung["sepa"]["MREF"]);
                ...
                */
                ...
                $buchung["kontoauszugdateiID"] = $this->kontoauszugdateiID;
                
                $umsatzID = $this->addBuchung($buchung);
        ...
        if (abs($this->delta - $deltaBank) > 0.001) {
